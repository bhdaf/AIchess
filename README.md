...
`distill` 子命令支持以下增强参数，提升中国象棋学生模型的实战强度。

### 7. 运行蒸馏训练（阶段 A）

#### 快速验证（10 局，mixed 轨迹 + game_outcome value）
...
python -m AIchess distill \
    ...
    --teacher_fallback_topk 3 \
    --teacher_fallback_temperature 0.6 \
    --distill_value_mode game_outcome \
    --eval_every_n_games 50 \
    --eval_n_games 20 \
    --eval_elo_list 500,800 \
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--trajectory_source` | `mixed` | 走子方来源：`weak`=弱引擎，`teacher`=强引擎，`mixed`=按比例混合 |
| `--mixed_teacher_ratio` | `0.7` | `mixed` 模式下 teacher 走子的概率（0~1） |

- `weak`：原有行为，弱引擎走子，teacher 只提供 policy 标注。
- `teacher`：强引擎走子，生成更高质量轨迹。
- `mixed`：以 `mixed_teacher_ratio` 概率选择 teacher 走子，否则弱引擎走子。

所有模式下 teacher 始终为所有局面提供 soft policy 监督。

#### 2) Teacher Soft Policy 质量控制

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--teacher_min_top1_prob` | `0.25` | teacher policy top1 概率下限，低于此值触发兜底 |
| `--teacher_max_entropy` | `0.9 * log(N)` | teacher policy 熵上限，超过此值触发兜底 |
| `--teacher_low_quality_action` | `fallback_topk_sharpen` | 低质量 policy 处理方式 |
| `--teacher_fallback_topk` | `3` | `fallback_topk_sharpen` 保留的 top-k 候选数 |
| `--teacher_fallback_temperature` | `0.6` | `fallback_topk_sharpen` 温度（< 1 更尖锐） |

`teacher_low_quality_action` 可选值：

| 值 | 说明 |
|----|------|
| `fallback_onehot` | 用 teacher 最优走法构造 one-hot 分布 |
| `fallback_topk_sharpen` | 仅保留 top-k 候选，温度锐化后归一化（**默认**） |
| `skip_sample` | 跳过本步样本，但对局继续 |

#### 3) Value Target 监督模式

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--distill_value_mode` | `game_outcome` | value 监督模式 |

| 模式 | 说明 |
|------|------|
| `zero` | 固定 0（与原有行为兼容，不训练 value head） |
| `game_outcome` | 按终局结果回填：红胜 +1，黑胜 -1，和棋 0，乘当前走棋方符号（**默认**） |

#### 4) 蒸馏评测闭环

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `--eval_every_n_games` | `200` | 每隔多少局触发一次快速对战评测（0=禁用） |
| `--eval_n_games` | `20` | 每次评测局数 |
| `--eval_elo_list` | `500,800` | 评测对手 Elo 列表（逗号分隔） |

评测结果自动追加写入运行目录的 `distill_eval_metrics.csv`。

#### 5) 训练可观测性（日志新增字段）

蒸馏对局日志（`self_play.jsonl` / `training_metrics.csv`）新增以下字段：

| 字段 | 说明 |
|------|------|
| `trajectory_source_mode` | 本局使用的轨迹来源模式 |
| `teacher_move_ratio` | teacher 走子步数占比 |
| `soft_policy_entropy_mean` | teacher policy 平均 Shannon 熵 |
| `soft_policy_low_quality_ratio` | 低质量 policy 样本比例 |
| `value_mode` | value 监督模式 |
| `effective_samples` | 本局有效训练样本数 |
| `skip_sample_count` | 本局因低质量跳过的样本数 |
